...
The majority of genomics studies is focussed on finding causal genetic variation in the protein-coding regions of the genome. However, these regions amount only to approximately two percent\cite{Lander2001}. One of the major reasons behind this is the relative uncomplicated nature of studying coding regions, as consequence on lower levels (e.g. transcription, proteins) are traceable. A good illustration of the complexity of the non-coding regions is the ENCyclopedia Of DNA Elements (ENCODE)-project\cite{ENCODE}, which contains over fifty different signals. 

The fact that non-coding regions often have roles in regulation of a distant genes (i.e. cis-acting), provides evens more complexity to the analysis of structural variants (SV) in these regions. For example the Pierre Robin Syndrome (PRS): structural variations (deletions or duplications) in the 3mB surrounding the SOX9-gene in specific tissues, are causative to the striking phenotype of undeveloped mandibles and tongue in children\cite{Benko2009,Kurth2009}. Studies on cancer-specific causative non-coding SV's are beginning to emerge in the last two years, including colorectal- and skin-cancer\cite{Ongen2014,Huang2013}. 
\medskip
...
While studies are starting to find the consequences of structural variations in coding regions (due to the relative ease of finding consequential changes in transcript and protein\cite{McLaren2010}), the first computational methods for non-coding regions are just starting to come up in the literature of 2014\cite{Khurana2013,Kircher2014}. The young nature of the used methods is visible in their output: only a prioritisation-score, based on the probability to be a causal variant, is given. Combining these scores with our linked-data methods -using both self-generated and public data- would lead to further understanding of the mechanisms of cancer. 
